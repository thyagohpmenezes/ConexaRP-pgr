// Generates a clean SurveyInput.tsx from scratch
const fs = require('fs');
      ...
      <div className="space-y-6">
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <div className="mt-8 text-center">
            <button disabled={!(isDoneEmployee && isDoneManager)}
              className={\`px-10 py-4 rounded-xl font-black uppercase text-sm tracking-widest transition-all shadow-xl active:scale-95 flex items-center gap-3 mx-auto \${(isDoneEmployee && isDoneManager) ? 'bg-slate-900 text-white hover:bg-slate-800' : 'bg-slate-100 text-slate-400 cursor-not-allowed shadow-none'}\`}>
              <FolderTree size={20} /> Gerar Triangulação Final
            </button>
            {!(isDoneEmployee && isDoneManager) && <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-4">* Complete os 3 eixos de dados acima para liberar a triangulação</p>}
          </div>
        </div>
      </div>
    );
...
console.log('✅ SurveyInput.tsx rewritten clean');
