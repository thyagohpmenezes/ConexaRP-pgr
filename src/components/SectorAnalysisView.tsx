...
import {
  ...
  ChevronDown, ChevronUp, TrendingUp, TrendingDown, Minus, Shield, Building2
} from 'lucide-react';
...
export default function SectorAnalysisView({ assessment, managerOverallMeanGlobal, checklistCriticalityGlobal, defaultExpandedAll }: SectorAnalysisViewProps) {
  ...
  const { sectorBreakdown, unitBreakdown, triangulationScore, riskScore } = assessment;
  
  // Normalizar dados para visualização
  const units = unitBreakdown && Object.keys(unitBreakdown).length > 0
    ? Object.entries(unitBreakdown)
    : sectorBreakdown && Object.keys(sectorBreakdown).length > 0
      ? [['MATRIZ', { name: 'MATRIZ', sectors: sectorBreakdown, rowCount: Object.values(sectorBreakdown).reduce((a, b) => a + (b.rowCount || 0), 0) }]]
      : [];
  ...
  const totalSectorsCount = units.reduce((a, [, u]) => a + Object.keys((u as any).sectors || {}).length, 0);
  const totalSample = units.reduce((a, [, u]) => a + ((u as any).rowCount || 0), 0);
  ...
  if (units.length === 0) {
    return (
      <div className="bg-white rounded-3xl border-2 border-dashed border-slate-200 p-16 text-center">
        ...
        <h3 className="text-lg font-black text-slate-700 uppercase tracking-tight mb-2">Nenhum dado analisado ainda</h3>
        <p className="text-sm text-slate-400 font-medium max-w-md mx-auto mb-6">
          Para ver o panorama organizacional, vá em <strong>DADOS → Eixos</strong> e realize os uploads mapeando as colunas de Unidade e Setor.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Triangulação Global', value: triangulationScore.toFixed(3), sub: 'Referência Empresa', color: 'text-blue-600' },
          { label: 'Unidades Ativas', value: units.length.toString(), sub: 'Filiais/Locais', color: 'text-slate-700' },
          { label: 'Setores Mapeados', value: totalSectorsCount.toString(), sub: 'Departamentos', color: 'text-slate-700' },
      ...
      <div className="space-y-6">
        {units.map(([unitName, unitData]: [string, any]) => {
          ...
          return (
            <div key={unitName} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              {/* Unit Header */}
              <div 
                onClick={() => units.length > 1 && setExpandedUnit(isUnitOpen ? null : unitName)}
                className={`px-6 py-4 flex items-center justify-between cursor-pointer transition-colors ${isUnitOpen ? 'bg-slate-900 text-white' : 'bg-slate-50 hover:bg-slate-100 text-slate-800'}`}
              >
                <div className="flex items-center gap-3">
                  <Building2 size={18} className={isUnitOpen ? 'text-blue-400' : 'text-slate-400'} />
                  <div>
                    <h3 className="text-xs font-black uppercase tracking-widest">Unidade: {unitName}</h3>
                    <p className={`text-[9px] font-bold uppercase tracking-widest ${isUnitOpen ? 'text-slate-400' : 'text-slate-500'}`}>
                      {sectors.length} Setores | {unitData.rowCount || 0} Colaboradores
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-6">
                  {unitData.unitOverallMean > 0 && (
                    <div className="hidden md:block text-right">
                      <p className="text-[8px] font-black uppercase opacity-60">Média Unidade</p>
                      <p className="text-xs font-black">{unitData.unitOverallMean.toFixed(2)}</p>
                    </div>
                  )}
                  {units.length > 1 && (isUnitOpen ? <ChevronUp size={20}/> : <ChevronDown size={20}/>)}
                </div>
              </div>

              {/* Sectors Table for this Unit */}
              {isUnitOpen && (
                  ...
                  <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {sortedSectors.map(([sName, sData]) => {
                        ...
                        const criticalDomains = (sData.domains || []).filter((d: DomainData) => d.employeeMean >= 3.0);
                        
                        return (
                          <React.Fragment key={sName}>
                            <tr className={`hover:bg-slate-50 transition-colors ${isSectorOpen ? 'bg-slate-50' : ''}`}>
                              <td className="px-6 py-4 text-xs font-bold text-slate-700 uppercase">{sName}</td>
                              <td className="px-4 py-4 text-center"><span className="font-black text-slate-700">{sData.rowCount || 0}</span></td>
                              ...
                              <td className="px-4 py-4 text-center">
                                <button onClick={() => setExpandedSector(isSectorOpen ? null : `${unitName}-${sName}`)} className="p-1.5 rounded hover:bg-slate-200 text-slate-500 transition-all">
                                  {isSectorOpen ? <ChevronUp size={14}/> : <ChevronDown size={14}/>}
                                </button>
                              </td>
                            </tr>
                            {isSectorOpen && (
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div>
                                      <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-2">
                                        <BarChart2 size={12} className="text-blue-500" /> Perfil de Domínios — {sName}
                                      </p>
                                      <div className="space-y-3">
                                        {(sData.domains || []).map((d: DomainData) => (
                                          <div key={d.id}>
